...
def AOFlag_AIPS(uvdata,sensitivity,profile,casa,ref_freq,AOsources,NonAOsources,intermediate):
	...
	for i in range(len(AOsources)):
		...
		os.system('aoflagger -strategy rfistrat_'+AOsources[i]+'_'+str(sensitivity[i])+'.rfis '+AOsources[i]+'_pre_AO.ms') #Aoflag the data
...
#Inputs for AOmodule
AIPS.userno = 3000                                  #AIPS usernumber the file you want to flag is located
uvdata = AIPSUVData('ALL','DQUAL',1,1)            #UV data file you want to flag in Parseltongue format i.e. AIPSUVData('NAME','CLASS',DISK,SEQ) 
AOsources = ['1555+3812','1557+3721']                            #Python list of the sources that you wish to flag with AOFlagger
NonAOsources = ['1407+284','1331+305']              #Python list of sources you don't want to flag (AOFLagger isnt great for bright sources)
...
intermediate_files = True                           #Delete intermediate working files if set to True

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #

AOFlag_AIPS(uvdata,sensitivity,profile,casa,ref_freq,AOsources,NonAOsources,intermediate_files)
